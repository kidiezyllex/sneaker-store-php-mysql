...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['add'])) {
        ...
        $db_conn->prepare("INSERT INTO brands (name, description) VALUES (?, ?)")->execute([$name, $description]);
        redirect('/admin/brands.php?success=1');
    }
    ...
    if (isset($_POST['update'])) {
        ...
        $name = sanitize($_POST['name']);
        $description = sanitize($_POST['description']);
        $db_conn->prepare("UPDATE brands SET name = ?, description = ? WHERE id = ?")->execute([$name, $description, $id]);
        redirect('/admin/brands.php?success=1');
    }
}

if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['delete_id'])) {
    $db_conn->prepare("DELETE FROM brands WHERE id = ?")->execute([$_POST['delete_id']]);
    redirect('/admin/brands.php?success=1');
}
                        ...
                        <?php foreach ($brands as $brand): ?>
                            <tr>
                                ...
                                <td class="table-actions">
                                    <button class="btn btn-sm btn-warning" data-bs-toggle="modal" data-bs-target="#editModal<?php echo $brand['id']; ?>">
                                    ...
                                    <div class="modal-content">
                                        ...
                                        <form method="POST">
                                            <div class="modal-body">
                                                ...
                                                <div class="mb-3">
                                                    <label class="form-label">Mô tả</label>
                                                    <textarea name="description" class="form-control" rows="3"><?php echo htmlspecialchars($brand['description'] ?? ''); ?></textarea>
                                                </div>
                                            </div>
